...
{library(dplyr)
  ...
  for (sequence_number in 1:length(sequence)) {
    ...
    if (fragments == "by") {
      ...
      ms1z1 <- rep(p1, times = (length(bi)+1))
      b1.type <- paste("b", bi, sep = "")
      c.type <- c(b1.type,"NA")
      y1.type <- paste("y", yi, sep = "")
      n.type <- c("NA",y1.type)
      seq <- cbind(peptide_vector)
    ...
    if (fragments == "cz") {
      ...
      ms1z1 <- rep(p1, times = (length(ci)+1))
      #c1.type <- as.data.frame(paste("c", ci, sep = ""))
    ...
    results_list[[sequence_number]] <- data.frame(ms1z1,Ion = c.type,c.type,C.monomass,seqAA=seq, Ion = n.type,n.type,N.monomass)%>%
      separate(c.type, c("Ion_type", "Ion_num"), sep = 1)%>% separate(n.type, c("Ion.type", "Ion.num"), sep = 1)
